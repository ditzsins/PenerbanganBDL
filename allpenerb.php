...
<?php
	...
	if(isset($_POST['simpan']))
	{
		...
		$query = "SELECT year(tgl_pesan), nama_maskapai, (SUM(total_harga)*5/100) FROM pesanan, detail_pesanan, jadwal, maskapai WHERE pesanan.id_pesanan = detail_pesanan.id_pesanan AND detail_pesanan.id_jadwal = jadwal.id_jadwal AND jadwal.kode_maskapai = maskapai.kode_maskapai AND year(tgl_pesan) BETWEEN $periode_tahun and $sampai_tahun GROUP BY year(tgl_pesan), nama_maskapai ORDER BY year(tgl_pesan) asc, (SUM(total_harga)*5/100) desc";
	...
	<div class = "container">
		...
		<div class="col-md-12" align="left">
			...
			<ul class="nav navbar-nav navbar-right" style="float: left; width: 20%;">
				<form action="allpenerbdownload.php" method="post" style="float: left; width: 10%;">
					<input type="hidden" name="query" value="<?php echo $query ?>" >
					<input type="image" src ="images/download.png" width = "40" alt="download" />
				</form>
				<a href="allpenerbgrafik.php" style="float: right; width: 80%;">
		...
		<div align = "center">
			<h2>Total Fee Mitra Penerbangan Per-Tahun</h2>
		</div>
		<br>
