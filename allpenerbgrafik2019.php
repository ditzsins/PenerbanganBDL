...
$query = sprintf("SELECT nama_maskapai, (SUM(total_harga)*5/100) as fee FROM pesanan, detail_pesanan, jadwal, maskapai WHERE pesanan.id_pesanan = detail_pesanan.id_pesanan AND detail_pesanan.id_jadwal = jadwal.id_jadwal AND jadwal.kode_maskapai = maskapai.kode_maskapai AND year(tgl_pesan) ='2019' GROUP BY year(tgl_pesan), nama_maskapai");
