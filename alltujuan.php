...
<?php
	...
	if(isset($_POST['simpan']))
	{
		...
		$query = "SELECT year(tgl_pesan), kota_tujuan, count(pesanan.id_pesanan) FROM pesanan, detail_pesanan, jadwal WHERE pesanan.id_pesanan = detail_pesanan.id_pesanan AND detail_pesanan.id_jadwal = jadwal.id_jadwal AND year(tgl_pesan) BETWEEN $periode_tahun and $sampai_tahun  GROUP BY year(tgl_pesan), kota_tujuan ORDER BY year(tgl_pesan) asc,count(pesanan.id_pesanan) desc, kota_tujuan asc";
	...
	<div class = "container">
		...
		<div class="col-md-12" align="left">
			...
			<ul class="nav navbar-nav navbar-right" style="float: left; width: 20%;">
				<form action="alltujuandownload.php" method="post" style="float: left; width: 10%;">
					<input type="hidden" name="query" value="<?php echo $query ?>" >
					<input type="image" src ="images/download.png" width = "40" alt="download" />
				</form>
				<a href="alltujuangrafik.php" style="float: right; width: 80%;">
		...
		<div align = "center">
			<h2>Jumlah Visit Tujuan Penerbangan Per-Tahun</h2>
		</div>
		<br>
