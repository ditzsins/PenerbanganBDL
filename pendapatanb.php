...
<?php
	...
	if(isset($_POST['simpan']))
	{
		...
		$query = "SELECT year(tgl_pesan), month(tgl_pesan), (SUM(total_harga) - (SUM(total_harga)*5/100)) FROM pesanan WHERE year(tgl_pesan) BETWEEN $periode_tahun and $sampai_tahun GROUP BY year(tgl_pesan), month(tgl_pesan)";
	...
	<div class = "container">
		...
		<div class="col-md-12" align="left">
			...
			<ul class="nav navbar-nav navbar-right" style="float: left; width: 20%;">
				<form action="pendapatanbdownload.php" method="post" style="float: left; width: 10%;">
					<input type="hidden" name="query" value="<?php echo $query ?>" >
					<input type="image" src ="images/download.png" width = "40" alt="download" />
				</form>
				<a href="pendapatanbgrafik.php" style="float: right; width: 80%;">
		...
		<div align = "center">
			<h2>Pendapatan Bersih Per-Bulan dan Per-Tahun</h2>
		</div>
		<br>
				...
				<tbody>
					...
					<?php while( $row = mysqli_fetch_assoc($result) ) : ?>
						<tr>
							...
							<td ><?php $mo = $row['month(tgl_pesan)']; echo $bulan[$mo]; ?></td>
							<td ><?php echo number_format($row['(SUM(total_harga) - (SUM(total_harga)*5/100))'],0) ?></td>
							<?php $i++;?>
							<?php endwhile;?>
						</tr>
				</tbody>
